...
def transcribe_local_video(video_path: str, output_dir: str = DEFAULT_OUTPUT_DIR, language: str = "auto"):
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title="Video Transcriber") as app:
        ...
        gr.Markdown(f"Enter a video URL or drag & drop a video file to transcribe it. Choose between local Whisper model or Groq API for transcription.")
            ...
            with gr.Column(scale=1):
                ...
                def validate_and_update_groq_client(api_key):
                    ...
                    try:
                        ...
                        client.models.list()
                        GROQ_CLIENT = client
                        return gr.update(), gr.update(value="✅ API key validated successfully!")
                    except Exception as e:
                        GROQ_CLIENT = None
                    error_type = "invalid_api_key" if "invalid_api_key" in str(e).lower() else "connection"
                    if error_type == "invalid_api_key":
                        error_msg = "❌ Invalid API key: Please check your API key and try again"
                    else:
                        error_msg = f"❌ Connection error: {str(e)}\nPlease check your internet connection and try again"
                    return gr.update(value="Local Whisper"), gr.update(value=error_msg, visible=True)
                ...
                def update_api_key_visibility(choice):
                    ...
                    status_value = "✅ Using configured API key" if is_groq and GROQ_CLIENT is not None else ""
                    
                    return gr.update(visible=key_visible), gr.update(visible=status_visible, value=status_value)
                ...
                model_choice.change(
                    ...
                    inputs=[model_choice],
                    outputs=[local_whisper_model, groq_model, groq_translate, groq_api_key]
                )
        ...
        def transcribe_with_groq(audio_path: str, language: str = "auto", model: str = "whisper-large-v3-turbo", translate: bool = False) -> dict:
                ...
                try:
                    ...
                    if translate:
                        ...
                        if model != "whisper-large-v3":
                            raise RuntimeError("Translation is only supported with the whisper-large-v3 model. The model has been automatically selected for you.")
                    ...
                    if isinstance(response, dict):
                        ...
                        segments = [{"start": 0, "end": len(transcription_text.split()) * 0.3, "text": transcription_text}]
        ...
        def handle_transcription(url, file, dir, lang, model_choice, local_model, groq_model, translate):
            if not file and not url:
                return "Please provide either a video URL or upload a video file.", None, None
            ...
            try:
                ...
                if model_choice == "Local Whisper":
                    ...
                    if GROQ_CLIENT is None:
                        return "Error: Groq API is not configured. Please check your API key configuration.", None, None
                        ...
                        if file is not None:
                            ...
                            try:
                                with yt_dlp.YoutubeDL({
                                    ...
                                    video_path_out, text_path, srt_path = save_output_files(
                                        dir, safe_title, result["text"], 
                                        generate_srt_content(result), video_path
                                    )
        ...
        transcribe_btn.click(
            ...
            inputs=[url_input, file_input, output_dir, language_dropdown, model_choice, local_whisper_model, groq_model, groq_translate],
